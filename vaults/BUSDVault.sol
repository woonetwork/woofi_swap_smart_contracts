...
contract mVault is ERC20, ERC20Detailed {
    ...
    mapping(address => uint256) public costSharePrices; // new
    ...
    function deposit(uint256 _amount) public payable onlyHuman {
        if (msg.value > 0) {
        ...
        if (totalSupply() == 0) {
            ...
            shares = (_amount.mul(totalSupply())).div(_pool); // fBusdAmount = _amount / (balance / totalSupply)
            uint256 _nowSharePrice = _pool.mul(1e18).div(totalSupply()); // new do not use balance() here cause totalSupply() not add until _mint
    ...
    function withdraw(uint256 _shares) public onlyHuman {
        uint256 r = (balance().mul(_shares)).div(totalSupply()); // sharePrice = balance() / totalSupply()

...
export class WooRouteHelper {
  ...
  constructor(_NETWORK_ID: number, _WEB3?: any, _ORACAL?: string, _WOOPP?: string) {
  ...
  public async getAllWooPairs(): Promise<WooPair[]> {
    ...
    let tmpInfo = web3.eth.abi.decodeParameters(
      ['uint112', 'uint112', 'uint32', 'uint64', 'uint64', 'uint112', 'bool'],
      rawQuoteInfo
    )
    let quoteTokenInfo = new WooppTokenInfo(
      new LocBN(tmpInfo[0]),
      new LocBN(tmpInfo[1]),
      new LocBN(tmpInfo[2]),
      new LocBN(tmpInfo[3]),
    ...
    for (let i = 0; i < this.BASETOKENS.length; ++i) {
      ...
      let baseTokenInfo = new WooppTokenInfo(
        new LocBN(tmpInfo[0]),
        new LocBN(tmpInfo[1]),
        new LocBN(tmpInfo[2]),
        new LocBN(tmpInfo[3]),
      ...
      let baseState = new WooracleState(
        new LocBN(tmpState[0]),
        new LocBN(tmpState[1]),
        new LocBN(tmpState[2]),
        tmpState[3]
      )

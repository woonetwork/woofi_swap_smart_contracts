...
contract StrategyLP is BaseStrategy {
    ...
    constructor(
        ...
        require(IPancakePair(initWant).token0() == lpToken0 || IPancakePair(initWant).token0() == lpToken1, 'StrategyLP: LP_token0_INVALID');
        require(IPancakePair(initWant).token1() == lpToken0 || IPancakePair(initWant).token1() == lpToken1, 'StrategyLP: LP_token1_INVALID');
